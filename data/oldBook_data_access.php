...
function editOldBook_DA($oldBook){
    $query="UPDATE tbl_oldBooks SET city='$oldBook[city]', area='$oldBook[area]', title='$oldBook[title]', description='$oldBook[description]', phone_number='$oldBook[phone_number]', price='$oldBook[price]', image_name='$oldBook[image_name]', added_date='$oldBook[added_date]', status='$oldBook[status]' WHERE id='$oldBook[id]' ";
    return executeNonQuery($query);
}
...
function getOldPublishedBooksList($id){
    $query="SELECT * FROM tbl_oldBooks WHERE status='reviewed' AND user_id='$id'";
    $result=executeQuery($query);
    $oldPublishedBooksList=array();
    if($result){
       for ($i=0; $row =mysqli_fetch_assoc($result) ; $i++) {
...
function getOldUnpublishedBooksList($id){
    $query="SELECT * FROM tbl_oldBooks WHERE (status='underReview' OR status='pending' OR status='edited') AND user_id='$id'";
    $result=executeQuery($query);
    $oldUnpublishedBooksList=array();
    if($result){
       for ($i=0; $row =mysqli_fetch_assoc($result) ; $i++) {
           $oldUnpublishedBooksList[$i]=$row;
       }
