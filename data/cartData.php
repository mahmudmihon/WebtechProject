<?php
	...
	{
		$query = "SELECT * FROM cart, cart.quantity AS cquantity LEFT JOIN newbooks ON newbooks.id=cart.product_id WHERE user_id=$id";

		$result=executeQuery($query);
		$total = 0;
		foreach ($result as $row) 
		{
			$subtotal = $row['price'] * $row['cquantity'];
			$total += $subtotal;
		}
	...
	function updateOrderData($id, $name, $phone, $aphone, $address, $method, $total, $orderId, $date)
	{
		...
		if($result)
		{
	       ...
	       { 
	           ...
	           $query2 = "SELECT quantity FROM newbooks WHERE id=$row[product_id]";

	           $totalQuan = executeQuery($query2);

	           $q1 = mysqli_fetch_assoc($totalQuan);
	           $q2 = $row['quantity'];
	           $newQuan = $q1['quantity'] - $q2;

	           $query3 = "UPDATE newbooks SET quantity=$newQuan WHERE id=$row[product_id]";
